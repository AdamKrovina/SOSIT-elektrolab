...
void setup()
{
  ...
  pinMode(motor1EndPin, INPUT_PULLUP);
  pinMode(motor2EndPin, INPUT_PULLUP);
  pinMode(motor2PresentPin, INPUT_PULLUP);
  pinMode(motor1PresentPin, INPUT_PULLUP);
  
  digitalWrite(motor1EnablePin, LOW); // enable motor

  lcd.init();
  lcd.backlight();
  lcd.setCursor(0,0);

  if(digitalRead(motor1PresentPin) == LOW) motor1Enabled = 1;
  if(digitalRead(motor2PresentPin) == LOW) motor2Enabled = 1;

  if(motor1Enabled){
    lcd.print("Homing motor1:");
    if(motorGoHome(&stepper1, motor1EndPin, HIGH) == 0){
      lcd.print("Done.");
    }else{
      lcd.print("Error");
      motor1Enabled = 0;
    ...
    lcd.print("Motor1 not present.");
  }
  
  lcd.setCursor(0,1);
  if(motor2Enabled){
    lcd.print("Homing motor2:");
    if(motorGoHome(&stepper2, motor2EndPin, HIGH) == 0){
      lcd.print("Done.");
    }else{
      lcd.print("Error");
      motor2Enabled = 0;
  ...
  if (parameters.Size() > 1) { // rele, 0/1
    ...
    if (parameters.Size() == 2){
      ...
      if(!onoff){
        ...
        relayOffTime[relay] = (unsigned long)Timeout;
        digitalWrite(relayPin[relay], HIGH);
       
      }else{
        relayOnTimestamp[relay] = millis();
        relayOnTime[relay] = (unsigned long)Timeout;
        digitalWrite(relayPin[relay], LOW);
      }
